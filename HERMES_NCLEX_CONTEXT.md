...
- Capture method updated after Emeka feedback: reviewer fills mobile form, drafts autosave in phone/browser localStorage, and the UI now prioritizes `Submit current review` plus `Submit all completed` instead of forcing GitHub Issues. Public GitHub Pages builds are configured to POST to `https://poiwatcher-backend.onrender.com/api/nclex/external-reviews`, where the existing Render backend appends submissions to the private Gist file `nclex-external-reviews.json`. The local review API also supports `/api/external-reviews` for development/local logging under `qbank_pipeline/external_review_submissions/` plus `qbank_pipeline/review_logs/external_review_submissions.jsonl`.
- Fallback if submit fails: the same buttons copy structured JSON to the clipboard; `GitHub fallback` still exists but is no longer the primary reviewer path.
